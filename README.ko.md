...
- **내부 스크롤 없는 9:16 모바일 로그라이크**와 PC 키보드·모바일 터치 조작. 규칙 등 화면을 넘는 내용은 내부 스크롤 대신 명시적 페이지로 나눕니다.
- 검토된 버전은 원본 ImageGen 카드 아트를 해시로 고정한 로컬 에셋으로 포함할 수 있으며 게임 프레임은 외부 이미지 URL을 불러올 수 없습니다.
- **영어 기본, 한국어 지원.** 한국 방문자는 처음에 한국어가 선택될 수 있으며, 직접 선택한 언어를 우선합니다.
- 로그인하지 않아도 의견을 보낼 수 있습니다. 익명 작성자는 영어에서 `anonymous`, 한국어에서 `ㅇㅇ`로 표시하며, 해당 의견 관리·투표·기여도 포인트 지급 대상이 아닙니다.
- 최근 60분 동안 신규 제안 최대 3개, 제안당 UTF-8 2,000바이트. 브라우저가 익명 횟수를 로컬에 기록하고 서버도 세션 한도를 별도로 강제합니다. 로컬 데이터 삭제를 신원 단위 초기화로 보장하지는 않습니다.
- Google 로그인은 의견 관리·투표·공개 별명 변경·기여도 리더보드에 사용합니다. 수정 가능한 로그인 제안의 수정은 신규 접수 횟수를 사용하지 않습니다.
- 버전별 로컬 세이브, 검토를 거치는 게임 공개, 후보 실패 시 마지막 검증 게임 유지.
- 모바일 게임 로더는 스트리밍 Fetch 본문이나 `HTMLImageElement.decode`가 없는 브라우저도 지원하고, 이미지 동시 전송을 제한하며, 첫 로드의 일시적 실패를 한 번 재시도하되 정상 게임을 교체하지 않습니다.
- 매일 **한국시간 23시 의견 마감·다음 자정 공개 목표**. 검토와 검증을 통과해야 하며, 정시 공개를 보장하는 일정은 아닙니다.
- Teen 수준을 넘지 않는 콘텐츠 기준. **정식 ESRB 등급을 취득했다는 뜻은 아닙니다.**

`v1-20260901`부터 시작한 검토 게임 버전들이 저장소에 포함되어 있습니다. Git에 파일이 있거나 빌드가 성공하거나 카운트다운이 끝났다는 사실만으로 게임 공개를 증명하지 않습니다. 실제 제공 선택과 운영자 검증으로 공개 상태를 판단합니다. 검증된 일일 공개 뒤에는 신뢰 워크플로가 실제 반영된 요구만 정확한 마감 투표 영수증과 대조해 자동 정산하며, 불변 성과 키로 중복 지급을 막습니다. 리더보드가 있다는 사실만으로 지급을 의미하지는 않습니다.
...
| `game-archive/` | 게임 데이터·당시 런타임 소스·로컬 에셋·무결성 목록을 누적하는 버전별 스냅샷 |
