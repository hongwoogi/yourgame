...
- PC·모바일 접속 지원. 지금은 흰 9:16 게임 미리보기 옆에 제안창을 배치하며 모바일은 아래에 표시. 실제 게임은 세로 화면·터치 조작 기준
- 메인에 최근·인기 의견 각 6개와 기여도 상위 10명 표시. 안전 승인과 작성자의 명시적 공개 동의가 있어야 의견을 노출하며, Google 신원 대신 별도 생성 별칭 사용
- 최초 모집 회차당 찬반 합산 활성 3표. 자기 투표 금지, 마감 전 변경·취소 가능. 리더보드 공개는 별도 동의하며 실제 게임 반영 전에는 점수를 발행하지 않음
...
[투표·기여도 기준](docs/voting-and-contribution.md)과 [커뮤니티 API](docs/community-api.md)는 공개 동의·최근/인기 의견·투표·기여도 원장 조회의 경계를 설명합니다. 기존 비공개 제안을 자동 공개하지 않습니다. 실제 게임 공개와 요구 충족을 검증하는 발행기가 연결되기 전에는 점수 지급을 차단합니다. 제안자 배점의 `**` 표기는 제곱인지 표당 가중치인지 확인 중이며 임의로 확정하지 않습니다.
