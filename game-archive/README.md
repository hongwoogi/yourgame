...
- In a Git checkout the same checks also protect snapshot files from `HEAD` and its parent against deletion or rewriting, even if the manifest was edited too. A clean `git archive` has no history and instead validates its complete manifest/file set. Keep this retention check in the pre-commit and pre-deployment workflow.
