...
(() => {
  ...
  const ANONYMOUS_HISTORY_KEY = 'yourgame.anonymous-submissions.v1';
  ...
  // Older clients queued a post-login send. Anonymous submission is now
  // explicit and direct, so never replay that stale browser intent.
  ...
  function anonymousHistory() {
    const cutoff = Date.now() - (limits().windowSeconds * 1000);
    const stored = readJson('localStorage', ANONYMOUS_HISTORY_KEY);
    const current = Array.isArray(stored) ? stored.filter(item => item && typeof item.requestId === 'string'
      && Number.isFinite(item.createdAt) && item.createdAt > cutoff && item.createdAt <= Date.now()) : [];
    writeStorage('localStorage', ANONYMOUS_HISTORY_KEY, current.length ? JSON.stringify(current) : null);
    return current;
  }

  function anonymousQuota() {
    const history = anonymousHistory();
    return { remaining: Math.max(0, limits().submissions - history.length), limit: limits().submissions,
      nextAvailableAt: history.length >= limits().submissions
        ? new Date(history[0].createdAt + limits().windowSeconds * 1000).toISOString() : null };
  }

  function recordAnonymousSubmission(requestId) {
    const history = anonymousHistory();
    if (!history.some(item => item.requestId === requestId)) {
      history.push({ requestId, createdAt: Date.now() });
      history.sort((a, b) => a.createdAt - b.createdAt);
      writeStorage('localStorage', ANONYMOUS_HISTORY_KEY, JSON.stringify(history));
    }
  }
  ...
  function limits() {
    return { bytes: status?.limits?.bytes || 2000, submissions: status?.limits?.submissions || 3,
      windowSeconds: status?.limits?.windowSeconds || 3600 };
  }
  ...
  function renderControls() {
    ...
    if (!loggedIn) {
      const localQuota = anonymousQuota();
      ui['quota-status'].textContent = t('quotaAnonymous', { name: t('anonymousName'),
        remaining: localQuota.remaining, limit: localQuota.limit });
      ui['quota-note'].textContent = localQuota.remaining === 0 && localQuota.nextAvailableAt
        ? t('quotaNext', { time: shortTime(localQuota.nextAvailableAt, true) }) : t('anonymousContributionNote');
    } else if (!quota) {
    ...
    const noQuota = loggedIn ? (!quota || quota.remaining <= 0) : anonymousQuota().remaining <= 0;
  ...
  async function sendAnonymousProposal(body, requestId) {
    if (submitting || composing || user || !validateBody(body)) return;
    if (!proposalsOpen()) {
      feedback(operatingState().proposalsPaused ? pausedSubmissionMessage() : m('notOpen'), 'error',
        { reason: operatingState().proposalsPaused ? 'service' : '' });
      return;
    }
    if (anonymousQuota().remaining <= 0) {
      feedback(m('anonymousNoQuota'), 'error', { reason: 'quota' });
      return;
    }
    submitting = true;
    clearPending();
    ...
    renderControls();
    try {
      const data = await request('/api/proposals', { method: 'POST', body: { body, requestId } });
      if (!data.proposal || data.anonymous !== true) throw new RequestError(m('resultUnknown'));
      recordAnonymousSubmission(requestId);
      newDraft = '';
      ui.prompt.value = '';
      writeStorage('localStorage', DRAFT_KEY, null);
      attempt = null;
      writeStorage('sessionStorage', ATTEMPT_KEY, null);
      feedback(m('anonymousSubmitted'));
      loadCommunity().catch(() => {});
    } catch (error) {
      const retryWait = error.retryAfterSeconds ? m('retrySeconds', { seconds: error.retryAfterSeconds }) : m('retrySoon');
      feedback(error.data?.error?.code === 'QUOTA_EXCEEDED' ? m('anonymousNoQuota')
        : error.data?.error?.code === 'PROPOSAL_ATTEMPT_RATE_LIMITED' ? m('attemptRateLimited', { wait: retryWait })
          : error.status === 0 ? m('sendUnknown') : errorMessage(error), 'error',
      { reason: error.status === 429 ? 'quota' : '' });
      if (isServiceRejection(error)) {
        applyServiceRejection(error);
        await refreshStatus().catch(() => {});
      } else if (error.status === 401 || error.status === 403) {
  ...
  ui['prompt-form'].addEventListener('submit', async (event) => {
    ...
    if (!user) {
      ...
      await sendAnonymousProposal(body, submissionAttempt(body).requestId);
      return;
    }
  ...
  function ideaAlias(idea) {
    return idea?.author?.anonymous === true ? t('anonymousName') : idea?.author?.alias || '';
  }
  ...
  function createCommunityCard(idea, source = 'main', index = 0) {
    ...
    alias.textContent = ideaAlias(idea);
    alias.title = ideaAlias(idea);
    ...
    read.setAttribute('aria-label', t('readIdeaAria', { alias: ideaAlias(idea) }));
  ...
  function openIdea(idea, sourceButton, source = 'main') {
    activeIdea = { id: idea.id, proposalRevision: idea.proposalRevision,
      ...
      alias: ideaAlias(idea), createdAt: idea.createdAt };
  ...
  function refreshIdeaDialog() {
    ...
    ui['idea-title'].textContent = t('ideaTitle');
    ui['idea-author'].textContent = ideaAlias(idea) + ' · ' + proposalDate(idea.createdAt);
    activeIdea.alias = ideaAlias(idea);
