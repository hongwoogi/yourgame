...
async function boundedAsset(response, asset) {
  if (!response.ok || !response.body || Number(response.headers.get('content-length')) > asset.bytes) throw new Error('GAME_ASSET_UNAVAILABLE');
  const reader = response.body.getReader(); const chunks = []; let total = 0;
  try {
    while (true) {
      const { done, value } = await reader.read(); if (done) break;
      total += value.byteLength;
      if (total > asset.bytes) throw new Error('GAME_ASSET_TOO_LARGE');
      chunks.push(value);
    }
  } finally { await reader.cancel().catch(() => {}); }
  if (total !== asset.bytes) throw new Error('GAME_ASSET_BYTES_CHANGED');
  const bytes = new Uint8Array(total); let offset = 0;
  for (const chunk of chunks) { bytes.set(chunk, offset); offset += chunk.byteLength; }
...
export function createGameHost({ surface, placeholder, note }) {
  ...
  async function mount(game, key) {
    ...
    try {
      ...
      const assets = await Promise.all((bundle.assets || []).map(async asset => boundedAsset(await fetch(
