...
export function validateGameBundle(input) {
  ...
  if (![1, 2, 3].includes(bundle.schemaVersion)) fail();
  exact(bundle, bundle.schemaVersion === 1 ? ['schemaVersion', 'config', 'copy', 'art']
    : bundle.schemaVersion === 2 ? ['schemaVersion', 'config', 'copy', 'art', 'assets']
      : ['schemaVersion', 'config', 'copy', 'art', 'assets', 'experience']);
  ...
  exact(bundle.art, bundle.schemaVersion === 1
    ...
    : bundle.schemaVersion === 2
      ...
      : ['background', 'panel', 'accent', 'ink', 'muted', 'heroIcons', 'cardImages', 'boardImage', 'heroImage']);
  ...
  if (bundle.schemaVersion >= 2) {
    const expectedAssets = bundle.config.cards.length + (bundle.schemaVersion === 3 ? 2 : 0);
    if (!Array.isArray(bundle.assets) || bundle.assets.length !== expectedAssets || bundle.assets.length > 16) fail();
    ...
    for (const asset of bundle.assets) {
      ...
      if (!ASSET_ID.test(asset.id || '') || !ASSET_PATH.test(asset.path || '') || asset.path !== folded
        || asset.mediaType !== 'image/png' || !Number.isSafeInteger(asset.bytes) || asset.bytes < 1 || asset.bytes > 3 * 1024 * 1024
        || !HASH.test(asset.sha256 || '') || !Number.isSafeInteger(asset.width) || !Number.isSafeInteger(asset.height)
        || asset.width < 64 || asset.height < 64 || asset.width > 1536 || asset.height > 1536
    ...
    if (bundle.schemaVersion === 3) {
      exact(bundle.art.boardImage, ['assetId']);
      exact(bundle.art.heroImage, ['assetId']);
      if (!assetIds.has(bundle.art.boardImage.assetId) || usedAssets.has(bundle.art.boardImage.assetId)
        || !assetIds.has(bundle.art.heroImage.assetId) || usedAssets.has(bundle.art.heroImage.assetId)
        || bundle.art.heroImage.assetId === bundle.art.boardImage.assetId) fail();
      usedAssets.add(bundle.art.boardImage.assetId);
      usedAssets.add(bundle.art.heroImage.assetId);
      exact(bundle.experience, ['choiceMode', 'founderId', 'rewardRule']);
      if (bundle.experience.choiceMode !== 'single-fixed'
        || !bundle.config.heroes.some(hero => hero.id === bundle.experience.founderId)
        || bundle.experience.rewardRule !== 'first-seeded') fail();
    }
