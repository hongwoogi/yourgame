...
현재 게임 개발용 안전 승인은 서버에서 관리자 권한을 확인한 명시적 검토로 발급한다. 접수 단계에서는 본문 내용 검사로 거절하지 않으며, 검토기가 위험 문구를 찾지 못했다는 이유만으로 개발용 자동 승인도 하지 않는다. 별도 의미 심사 모델과 격리된 검토 실행자가 연결·검증되기 전에는 자동 검토가 있다고 표시하지 않는다. 검토 대기는 운영 기록과 관리자 화면에서 확인하고 첫 게임 공개 목표를 이유로 승인 절차를 생략하지 않는다. 웹 피드 노출은 이 승인에 종속되지 않는다.
