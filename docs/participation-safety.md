...
상태: 2026-08-31의 최신 사용자 요청을 반영한 게임 개발 안전 기준과 구현 계약. 미국 ESRB Teen을 게임 개발 상한으로 삼고, 과도한 선정성·폭력 및 사용자 입력을 통한 운영 규칙 무력화를 제한한다. 제안의 웹 공개·투표는 기본 공개 정책에 따라 사전 내용 필터나 안전 승인 없이 제공하며, 게임 개발 입력·채택·기여도 발행의 안전 판단과 분리한다. 실제 게임, 격리된 자동 생성 실행자와 신뢰된 산출물 검토 발급 경로는 아직 없다. 이 문서나 입력 검사 통과를 정식 등급 취득·완전한 공격 차단·게임 공개 성공으로 표시하지 않는다.
...
메인 화면에는 Teen 목표 안내나 안전 심사 상태를 표시하지 않는다. 정식 심사를 거치지 않은 상태에서 ESRB 인증 마크나 “Teen 등급 취득”을 표시하지 않으며 게임 개발 내부 기준은 유지한다. 실제 등급은 별도 절차이며, 콘텐츠 변경 뒤에도 다시 확인해야 한다. [ESRB 등급 절차](https://www.esrb.org/ratings/ratings-process/)
...
flowchart TD
  ...
  B -->|요청 유효| E[접수 · 원문 버전 보존]
  B -->|요청 무효| D[접수 거절 · 초안 유지]
  E --> P[기본 공개 · 즉시 투표 가능]
  E --> F{현재 내용과 정책에 대한 안전 심사}
  F -->|판단 보류·금지| G[게임 개발·채택·보상 제외]
...
| 상태 | 의미 | 개발 입력 사용 |
| --- | --- | --- |
| 접수 거절 | 권한·크기·빈도·운영 상태 등 요청 조건을 충족하지 않음. 새 제안·수정 저장 없음 | 불가 |
...
게임 개발용 안전 승인은 서버에서 관리자 권한을 확인한 명시적 검토로 발급한다. 2026-08-31 사용자가 Codex의 개별 심사·승인 및 운영 DB 처리를 위임한 이후에는 [위임받은 로컬 운영자 심사](admin.md#위임받은-로컬-운영자-안전-심사)도 허용한다. 실제 원문별 의미 검토와 정리문, 현재 연결 확인, 위임 검토자 감사 기록이 필요하며 Google 로그인으로 사칭하지 않는다. 접수 단계에서는 본문 내용 검사로 거절하지 않으며, 위험 문구 미검출만으로 자동 승인하지 않는다. 별도 상시 의미 심사 모델과 격리된 검토 실행자가 연결·검증되기 전에는 그런 자동화가 있다고 표시하지 않는다. 첫 게임 공개 목표를 이유로 심사를 생략하지 않으며 웹 피드 노출은 승인에 종속되지 않는다.
...
승인 뒤 수정하면 이전 승인·정리문은 새 본문에 유효하지 않다. 기존 제안, 원문 버전과 심사 이력은 남기되 새 revision은 개발용 검토 대기로 간다. 유효한 수정은 즉시 공개하고 이전 본문에 대한 표는 새 본문으로 옮기지 않는다. 형식·권한 등으로 거절된 수정은 저장하지 않는다. 기본 공개 전환은 기존 미선택 제안에도 적용하지만 과거 명시적 비공개 선택은 보존한다. 이를 작성자의 동의나 안전 승인으로 기록하지 않는다. 정책 버전이 바뀌면 과거 개발 승인을 최신 정책 승인으로 재사용하지 않는다.
...
- 형식·권한·운영 조건 등으로 서버가 접수를 거절하면 신규 제안 3회 한도를 차감하지 않는다. 브라우저 초안과 오류 안내를 유지하고 자동 재전송하지 않는다. 제안 내용의 사전 심사는 접수 조건이 아니다.
...
| 원문 → 화면·검토 | 공개 여부와 무관하게 신뢰하지 않는 데이터로 취급. 화면에는 textContent로 원문을 표시하며 HTML·Markdown 실행, 링크 자동 열기, 첨부 코드 실행, 환경변수 삽입 금지 |
...
[투표와 기여도 설계](voting-and-contribution.md)를 따른다. 기본 공개 정책으로 신규·수정 제안을 즉시 피드에 표시하고 투표할 수 있게 한다. 공개 동의 창, 사전 안전 승인, 메인 화면의 Teen 안내는 제거한다. 자동 노출의 근거는 `service_default`이며 작성자가 동의했다고 감사 이력을 만들지 않는다. 기존 미선택 제안은 전환 대상이지만 과거 명시적 비공개 선택과 관리자 제외·회원 제한은 보존한다. Google 이름·이메일·sub·사진을 공개 별명이나 리더보드 정보로 재사용하지 않는다.

게임 안전 심사의 `pending`·`held`·`blocked`만으로 공개나 표를 소멸시키지 않으며 승인 후에도 유효한 기존 표를 유지한다. 본문·공개 세대·회원 제어·관리자 제외가 바뀌면 이전 표는 재사용하지 않는다. 실제 게임 입력으로 선택할 때는 현재 본문과 정책에 대한 개발용 승인을 별도로 검사한다. 관리자 제외는 사후 운영 수단이며 삭제 기능이 아니다.
...
| 운영 명령·비밀 요청·가짜 시스템 역할·규칙 무시 | 텍스트로 접수·공개하더라도 실행·권한 변경 없음. 명백한 금지 요구는 게임 개발용 승인 불가 |
| 제로폭·전각·혼합 언어·인코딩·여러 제안에 분산 | 분석용 정규화와 검토, 탐지 누락도 승인 없이 개발 불가 |
| 신규·수정 동시 다발, 승인과 수정 경쟁 | 제출/시도 제한과 revision 검사, 옛 승인 재사용 없음 |
| 안전 승인 없이 운영 `reviewed`만 설정 | 웹 공개·투표 가능, 게임 개발 입력으로 사용 불가 |
...
접수·공개 피드·투표·관리자·입력 스냅샷에 관한 검사는 코드 테스트와 연결한다. 실제 게임의 의미·실행 격리 검증, 신고 전용 화면·신뢰된 기여도 발행은 아직 없는 기능의 완료 항목으로 세지 않는다.
