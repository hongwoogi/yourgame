...
`GET /api/community` 또는 `?view=public`은 익명으로 조회할 수 있다. 세션을 만들거나 Google 이름·이메일을 공개하지 않는다. 이 조회에는 다른 쿼리나 중복 `view`를 허용하지 않는다. 최근 의견과 인기 의견은 각 최대 6개, 호환용 리더보드 응답은 최대 10명이며 메인 화면은 그중 상위 5명만 표시한다.
