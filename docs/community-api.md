...
제안은 별도 공개 동의나 안전 승인 없이 저장 직후 공개되고, 열린 실제 회차에서는 로그인 참여자가 투표할 수 있다. 접수 단계에 콘텐츠 규칙 필터를 적용하지 않는다. 신규 제안은 로그인 없이도 `POST /api/proposals`로 보낼 수 있으며 UTF-8 2,000바이트·최근 60분 신규 3개·운영 제어를 유지한다. 브라우저 로컬 횟수는 즉시 안내를 위한 보조 수단이고 서버가 익명 세션별 이동 한도를 원자적으로 다시 검사한다. 게임 표시는 실제 검증된 공개 상태를 따르며, 검증된 게임이 없으면 `/api/status`의 `game.published=false`와 준비 화면을 유지한다. UI는 [언어 지원 기준](localization.md)을 따른다.

## 익명 제안

익명 세션의 `POST /api/proposals`는 같은 출처와 유효한 CSRF 토큰, `body`, `requestId`를 요구한다. 같은 세션·요청 ID·본문의 재전송은 한도를 추가 소비하지 않는 동일 응답이며, 같은 요청 ID의 다른 본문은 충돌로 거절한다. 공개 작성자는 영어 `anonymous`, 한국어 UI `ㅇㅇ`로 표시한다. 익명 제출은 로그인 계정이 아니므로 내 의견 조회·수정·공개 회수·투표·별명·개인 순위 기능을 제공하지 않으며 작성자 기여도 포인트를 발행하지 않는다. 브라우저 저장소나 쿠키 삭제를 자연인 단위 한도 초기화 또는 우회 방지로 보장하지 않는다.
