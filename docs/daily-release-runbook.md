...
3. 신뢰 운영자만 `exportReviewIntake(client, 'pending')`를 사용해 비공개 원문 검토 자료를 얻고, 해당 회차의 created_at 범위로 ID 집합을 제한한다. `scripts/operator-safety-review.mjs`의 정확한 계약을 읽어 Teen·개인정보·프롬프트 주입/운영 권한을 의미 심사한다. **안전 판단과 구현 가능성을 분리한다.** 안전한 요구를 현재 런타임에 없거나 원본 생성 이미지가 필요하다는 이유만으로 held 처리하지 않는다. 구현 가능성·범위는 승인된 독립 개발 정리문을 고정한 뒤 제작 단계에서 판단한다. 본문·비밀값·binding 지문은 콘솔과 Git에 출력하지 않는다. `applyOperatorReview`는 현재 revision/hash에 묶인 실제 판단을 기록하는 도구이며 자동 승인 판정기가 아니다. 안전 의미가 모호하면 held, 명확한 정책 위반이면 blocked로 남긴다. held 오판을 바로잡을 때는 같은 본문·정책·review revision에 묶인 별도 의미 재심사와 감사 후 `applyOperatorHeldReReview`를 사용하며 blocked를 우회하지 않는다.
...
9. 승인 요구가 원본 생성 이미지 에셋을 포함하면 신뢰 운영자가 정리문에서 제품 아트 요구만 추출해 내장 ImageGen을 호출한다. 참여자 원문·URL·명령·DB·비밀·배포 권한은 넘기지 않는다. 생성 PNG는 run의 `output/step03_assets/`에 두고 asset manager가 path·MIME·bytes·SHA-256·dimensions·provenance를 기록한다. 외부 런타임 URL은 금지하며 `prepare-game-candidate.mjs --assets=...`가 manifest와 실제 파일을 대조해 source/assets digest에 묶는다. 생성 성공은 안전 승인이나 공개 승인이 아니다.
10. 신뢰 운영자가 후보의 의미·요구 충족·Teen·EN/KO·원작/라이선스·실제 게임 동작을 검토한다. `prepare-game-candidate.mjs`, `validate-game-candidate.mjs`, `npm run build`, 관련 전체 UI 검증을 수행한다. 9:16 터치/키보드, 승패/보상, 일시정지, 새로고침 저장, 버전 간 격리와 인증/admin 접근 차단을 확인한다. 게임의 menu/play/help/reward/growth/pause/result 각 페이지에서 가로·세로 overflow가 없고, 넘치는 내용은 스크롤 대신 명시적 이전/다음 페이지로 도달 가능해야 한다. 전체화면은 9:16 중앙 letterbox를 유지한다. 로컬 이미지의 실제 표시·dimensions·served bytes·외부 요청0건도 확인한다. 검토 후 `createGameReleaseStore(client).issueReview(...)`로 정확한 스냅샷·후보·런타임·자산·실행 revision·증거에 결합된 영수증을 발급한다.
11. 커밋된 정확한 파일만 새 `.local/` 아카이브에 추출해 `npm ci`와 `npm run build`를 다시 실행한다. Windows에서는 `git -c core.autocrlf=false -c core.eol=lf archive ... HEAD`로 검토한 LF 바이트를 보존하고 추출 후 해시를 대조한다. 머신 전역 Git 설정을 바꾸지 않는다. 자정 전 후보 파일은 비공개로 유지하고 main push/운영 후보 자산 배포·공개 선택을 하지 않는다.
