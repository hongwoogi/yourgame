...
상태: 공식 문서 조사 후 Turso 무료 플랜을 선택했다. 2026-08-31 사용자 약관 승인 후 Vercel Marketplace를 통해 yourgame DB를 생성하고 production 환경에 연결했다. 선택된 플랜은 Starter 월 $0다. 유료 플랜은 선택하지 않았다.
...
비교 당시 추천은 **Turso Free**, 대안은 **Neon Free**였으며 이후 사용자가 GitHub·Vercel·Turso Free 구성을 수락했다. 실제 Vercel CLI의 tursocloud/database 연동에서는 무료 플랜이 Starter, plan ID starter, 월 $0로 표시됐다. 도쿄 hnd1, 이름 yourgame, production 연결을 지정해 생성했고 resource inspect에서 Starter 및 production 연결을 확인했다. 받은 production 환경변수로 로컬 SELECT 1 쿼리도 성공했다. 기존 직접 가입 Turso 계정과 Marketplace 계정의 관계 및 세부 사용량 한도는 아직 별도로 확인하지 않았다. 아래 직접 가입 요금표의 한도를 현재 Marketplace DB에 그대로 적용한다고 가정하지 않는다.
