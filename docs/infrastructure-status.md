...
- 서버·코어 220개와 Playwright 100개, 합계 320개 검사가 통과했다. 네이티브 DB 연결 3개에서 12개 동시 투표 중 정확히 3개만 성공했다. 교차검토에서 계정 불일치 후 개인정보 잔류, 다음 회차 미정 DTO, 429·깨진 성공 응답 후 재확인 요청 보존 문제를 찾아 배포 전에 수정하고 회귀로 확인했다. UI의 로그인 검사는 Google/API 대역이며 실계정 인증과 다르다.
