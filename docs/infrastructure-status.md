...
## 전체 의견 모달·배포 시각·게임 제작팀

- 2026-08-31 후속 요청으로 공개 의견의 최근·인기순 전체 탐색 API와 24개 단위 큰 모달, 남은 총 시간·분·초 카운트다운, 영어 워싱턴 D.C./한국어 한국시간 배포 목표 표시를 추가한다. 실제 첫 공개 목표의 UTC 값과 기존 투표·인증·공개 범위는 유지한다. 이 변경에는 운영 DB 마이그레이션이 없다.
- 프로젝트의 `.codex/agents/`에 오케스트레이션·시나리오·아트·게임 알고리즘·에셋 관리 역할을 정의하고 [인계 및 검증 절차](game-agent-workflow.md)를 연결한다. 모델은 상위 작업 설정을 상속한다. 역할 정의와 구조 검사는 실제 격리 실행 또는 게임 공개 성공을 증명하지 않는다. 현재 게임·기여도 발행 게이트를 우회하지 않는다.
- 로컬 전체 검사는 Node 309개, UI 134개, 팀 구조 회귀 18개로 총 461개를 통과했다. 45초 배경 갱신 중 동일 원문을 계속 읽는 동작, 변경/철회/조회 실패 시 원문 회수, 로그인 창 언어 변경 후 취소 초점, 비메인 의견 투표·중복 요청·계정 전환, EN/KO의 PC·모바일 모달 배치를 포함한다. Codex의 읽기 전용 `config/read`에서 프로젝트 설정과 하위 작업 상한 3개도 확인했다. 이 검사는 실제 계정 인증이나 게임 역할의 실제 생성 실행을 포함하지 않는다.
- 이번 변경의 데이터 보존, 커밋 파일만으로 수행한 설치·빌드, 실제 배포 자산·의견 페이지·익명 모달·권한 거부·health/runtime 검증은 `.local/ideas-agents-release-verification.json`에 별도로 기록한다. 이 문서가 있거나 Git에 push되었다는 사실만으로 검증 완료로 취급하지 않는다. 이전 공개 전환·별명 확장의 검증 기록은 보존한다.

## 공개 별명 변경·내 순위·전체 순위

- 2026-08-31 후속 요청은 공개 별명 변경, 메인 상위 5명과 여섯 번째 내 순위, 전체 순위 모달, 내 의견 배지와 본인 투표 금지다. 영어·한국어, 16:9 데스크톱 한 화면과 모바일 한 줄 헤더를 유지한다. 전체 순위는 20명씩 읽으며 동점은 같은 순위다.
- 기존 생성 별명과 공개 식별자를 바꾸지 않고 별도 표시 이름을 저장한다. 같은 별명도 허용하지만 소유권은 고정된 식별자로 판정한다. 변경은 본인 로그인·CSRF·프로필 revision·멱등 요청·기존 요청 빈도 제한과 운영 제어를 거치며 투표·점수·Google 계정 정보를 변경하지 않는다.
- 전용 스키마 준비는 표시 이름 테이블과 메타만 추가하며 기존 기본 공개 정책을 다시 활성화하거나 일반 초기화를 실행하지 않는다. 배포 전에 커밋 파일만 설치·빌드하고 준비한다. 변경 전후 보존, 실제 배포와 자산 일치, 전체 순위 조회·실제 익명 UI·권한 거부·health/runtime 결과는 `.local/profile-leaderboard-release-verification.json`에 따로 기록한다. 이전 공개 전환 증거를 덮어쓰지 않는다.
- 변경 검증은 Node 294개와 UI 123개, 총 417개를 통과했다. 이름 변경의 중복 요청·동시 수정·원자적 기록, 동점과 전체 순위, 계정 전환·만료, 열린 순위 모달의 최신 공개 범위 반영, 영어·한국어의 PC/모바일 배치를 포함한다. 운영 배포 결과는 위 별도 증거 기록으로 확인한다.
- 인증된 별명 변경·내 순위·동명이인 투표 방지의 로컬 검사는 운영 실제 계정의 로그인 성공을 대신하지 않는다. Google 버튼 표시, 실제 Google 계정 인증, 제안/별명 쓰기, 게임 공개·기여도 발행을 각각 구분한다.

## 기본 공개·간결한 데스크톱 화면

- 2026-08-31 후속 요청은 제안 접수와 웹 공개를 사전 내용 필터·공개 동의·개발 안전 승인에 종속시키지 않는 것이다. 기본 공개의 근거는 `service_default`로 별도 기록하며 작성자가 동의했거나 안전 검토가 끝났다고 표시하지 않는다. 게임 개발용 현재 본문 승인·격리 실행·산출물 검증은 계속 필요하다.
- 데스크톱 16:9는 게임 미리보기·참여 영역·리더보드 3열로 배치한다. 투표는 입력창 위, 최근·인기 목록은 각 6개를 3개씩 페이지로 표시하며 원문·내 제안은 모달로 읽는다. Teen과 공개 동의 안내는 제거하고 모바일 한 줄 상단바·국기 선택·`/master` 권한은 유지한다. 오류·긴 원문을 숨겨 한 화면으로 보이게 하지 않는다.
- 11:42:52 KST 변경 전 읽기 전용 기준은 회원 5명, 제안·본문 이력·심사 각 3건, 검토 대기 3건·승인 0건, 기여도 원장 0건, 프로필 1건이며 공개 세대·표·명시적 선택 이력은 0건이었다. 현재/이력 본문의 제한된 연락처·credential 형태 검사에서 일치 0건을 확인했지만 내용 안전 승인이나 완전한 개인정보 부재의 증거는 아니다. 원문은 출력하지 않았다.
- 기본 스키마 버전 1을 유지하면서 inactive 준비와 명시적 활성화·기존 미선택 데이터 전환을 분리한다. 기존 명시적 숨김·본문·계정·안전 심사·운영 제어·점수 원장은 보존한다. 관련 검사, 커밋 단독 설치·빌드, 운영 배포 일치·실제 공개 피드·UI·데이터 보존 결과는 `.local/public-default-release-verification.json`에 별도로 남긴다. Git push나 HTTP 200만으로 성공을 판단하지 않는다.
- 아래 항목들은 이전 배포의 역사적 기록이다. 사전 공개 동의·안전 승인 조건은 이번 기본 공개 정책으로 대체되며, 그때의 실제 검사 결과와 당시 미검증 사항은 변경하지 않는다. 실제 Google 계정 인증과 게임 공개는 여전히 별도 검증 대상이다.
