...
let dragCard = null, dragPointer = null, dragOrigin = null, suppressCardClick = null;
...
function cardButton(id, reward = false, index = 0) {
  ...
  const node = button('', () => {
    if (suppressCardClick === id) { suppressCardClick = null; return; }
    reward ? chooseReward(id) : playCard(id);
  }, reward ? 'reward' : 'card');
  ...
  if (!reward) {
    node.addEventListener('pointerdown', event => {
      ...
      dragCard = id; dragPointer = event.pointerId; dragOrigin = { x: event.clientX, y: event.clientY };
      try { node.setPointerCapture(event.pointerId); } catch { /* Synthetic QA pointers and older browsers may not capture. */ }
    });
    node.addEventListener('pointerup', event => {
      if (dragPointer !== event.pointerId || dragCard !== id || !dragOrigin) return;
      const moved = Math.hypot(event.clientX - dragOrigin.x, event.clientY - dragOrigin.y) >= 8;
      const target = moved ? document.elementFromPoint(event.clientX, event.clientY)?.closest('.hex') : null;
      dragCard = null; dragPointer = null; dragOrigin = null;
      if (!target || target.disabled) return;
      suppressCardClick = id;
      selectedTile = Number(target.dataset.tile);
      if (card.effect.defense) take({ type: 'play', cardId: id, tileId: selectedTile }); else playCard(id);
    });
    node.addEventListener('pointercancel', () => { dragCard = null; dragPointer = null; dragOrigin = null; });
  }
  return node;
}
function render() {
  ...
  if (menu) {
    ...
    const availableHeroes = bundle.experience?.choiceMode === 'single-fixed'
      ? bundle.copy.heroes.filter(hero => hero.id === bundle.experience.founderId) : bundle.copy.heroes;
    const heroPages = Math.ceil(availableHeroes.length / 4);
    const heroImage = bundle.art.heroImage;
    if (heroImage && assetUrls.has(heroImage.assetId)) {
      const portrait = element('img', 'founder-portrait'); portrait.src = assetUrls.get(heroImage.assetId);
      portrait.alt = ''; portrait.width = 1254; portrait.height = 1254; root.append(portrait);
    }
    for (const hero of availableHeroes.slice(heroPage * 4, heroPage * 4 + 4)) {
  ...
  if (state.phase === 'victory' || state.phase === 'defeat') {
    ...
    const rewardChoices = bundle.experience?.rewardRule === 'first-seeded' ? state.rewardChoices.slice(0, 1) : state.rewardChoices;
    rewardChoices.forEach(id => rewards.append(cardButton(id, true))); root.append(rewards);
  } else {
    ...
    for (let tileId = 0; tileId < 7; tileId++) {
      ...
      const boardBinding = bundle.art.boardImage;
      if (boardBinding && assetUrls.has(boardBinding.assetId)) hex.style.backgroundImage = `url("${assetUrls.get(boardBinding.assetId)}")`;
